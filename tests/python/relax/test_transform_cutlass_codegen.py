...
def build(mod):
    ...
    return True
    """
    executable = relax_build(mod, "llvm")
...
def constructGEMM(m, n, k):
    ...
    class GEMM:
        @T.prim_func
        def gemm(x: T.handle, y: T.handle, z: T.handle) -> None:
        ...
        def main(A: R.Tensor((m, k), A_TYPE), B: R.Tensor((k, n), B_TYPE)):
            with R.dataflow():
                C: R.Tensor((m, n), C_TYPE) = R.call_tir(gemm, (A, B), R.Tensor((m, n), C_TYPE))
                R.output(C)
            return C
...
# einsum "ghij, ghjk -> ghik", with a bias of shape (1, k)
def constructMultiBatchGEMM_bias(b1, b2, m, n, k, GLOBAL_SYMBOL="MultiBatchHGEMM_bias"):
    from tvm.script.ir_builder import IRBuilder
    from tvm.script.ir_builder import ir as I
    from tvm.script.ir_builder import relax as R
    from tvm.script.ir_builder import tir as T
    ...
    with IRBuilder() as ib:  # pylint: disable=invalid-name
        with I.ir_module() as frame:
            with T.prim_func():
                ...
                C = T.arg("C", T.buffer_decl((b1, b2, m, n), C_TYPE))
                D = T.alloc_buffer((b1, b2, m, n), C_TYPE)
                with T.grid(b1, b2, m, n, k) as (lb1, lb2, l0, l1, l2):
                    with T.block("multi_batch_dense_row_row_row"):
                        vb1, vb2, vi, vj, vk = T.axis.remap("SSSSR", [lb1, lb2, l0, l1, l2])
                ...
                with T.grid(b1, b2, m, n) as (lb1, lb2, l0, l1):
                    with T.block("bias"):
                        vb1, vb2, vi, vj = T.axis.remap("SSSS", [lb1, lb2, l0, l1])
                        T.reads(D[vb1, vb2, vi, vj], bias[0, vj])
                        T.writes(C[vb1, vb2, vi, vj])
                        T.buffer_store(C, D[vb1, vb2, vi, vj] + bias[0, vj], [vb1, vb2, vi, vj])
            with R.function():
                R.func_name("main")
                A = R.arg("A", R.tensor((b1, b2, m, k), A_TYPE))  # pylint: disable=invalid-name
                B = R.arg("B", R.tensor((b1, b2, k, n), B_TYPE))  # pylint: disable=invalid-name
...
def test_cutlass_multi_batch_dense_bias():
    b1, b2, m, n, k = 2, 3, 128, 128, 128
    assert build(constructMultiBatchGEMM_bias(b1, b2, m, n, k)), "build failure on CUDA"
    dev = tvm.cuda()
    A = np.random.rand(b1, b2, m, k).astype("float16") * 5
    B = np.random.rand(b1, b2, k, n).astype("float16") * 5
    bias = np.random.rand(1, n).astype("float16") * 5
...
# einsum "ghij, hjk -> ghik", with a bias of shape (1, k)
def constructMultiBatchGEMM2_bias(b1, b2, m, n, k, GLOBAL_SYMBOL="MultiBatchHGEMM2_bias"):
    from tvm.script.ir_builder import IRBuilder
    from tvm.script.ir_builder import ir as I
    from tvm.script.ir_builder import relax as R
    from tvm.script.ir_builder import tir as T
...
def test_cutlass_multi_batch_dense2_bias():
    b1, b2, m, n, k = 2, 3, 128, 128, 128
    assert build(constructMultiBatchGEMM2_bias(b1, b2, m, n, k)), "build failure on CUDA"
    dev = tvm.cuda()
    A = np.random.rand(b1, b2, m, k).astype("float16") * 5
    B = np.random.rand(b2, k, n).astype("float16") * 5
    bias = np.random.rand(1, n).astype("float16") * 5
...
# einsum "ij, ik -> jk", with a bias of shape (1, k)
def constructTransGEMM_bias(m, n, k, GLOBAL_SYMBOL="TransHGEMM_bias"):
    from tvm.script.ir_builder import IRBuilder
    from tvm.script.ir_builder import ir as I
    from tvm.script.ir_builder import relax as R
    from tvm.script.ir_builder import tir as T
...
def test_cutlass_trans_dense_bias():
    m, n, k = 128, 128, 128
    assert build(constructTransGEMM_bias(m, n, k)), "build failure on CUDA"
    dev = tvm.cuda()
    A = np.random.rand(k, m).astype("float16") * 5
    B = np.random.rand(k, n).astype("float16") * 5
    bias = np.random.rand(1, n).astype("float16") * 5
    ...
    executable = tvm.runtime.load_module(PKG_FILE)
    result = f_run(executable, dev, A_tvm, B_tvm, bias_tvm)
    np.testing.assert_allclose(result.numpy(), A.T @ B + bias, rtol=1e-2)
...
# einsum "hij, hjk -> ik", with a bias of shape (1, k)
def constructReductionGEMM_bias(b, m, n, k, GLOBAL_SYMBOL="ReductionHGEMM_bias"):
    from tvm.script.ir_builder import IRBuilder
    from tvm.script.ir_builder import ir as I
    from tvm.script.ir_builder import relax as R
    from tvm.script.ir_builder import tir as T
...
def test_cutlass_reduction_dense_bias():
    b, m, n, k = 2, 128, 128, 128
    assert build(constructReductionGEMM_bias(b, m, n, k)), "build failure on CUDA"
    dev = tvm.cuda()
    A = np.random.rand(b, m, k).astype("float16") * 5
    B = np.random.rand(b, k, n).astype("float16") * 5
    bias = np.random.rand(1, n).astype("float16") * 5
    ...
    executable = tvm.runtime.load_module(PKG_FILE)
    result = f_run(executable, dev, A_tvm, B_tvm, bias_tvm)
    np.testing.assert_allclose(result.numpy(), np.einsum("hij, hjk->ik", A, B) + bias, rtol=1e-2)
...
# einsum "hij, kj -> hik"
def constructPermutationGEMM2(b, m, n, k, GLOBAL_SYMBOL="PermutationHGEMM2"):
    from tvm.script.ir_builder import IRBuilder
    from tvm.script.ir_builder import ir as I
    from tvm.script.ir_builder import relax as R
    from tvm.script.ir_builder import tir as T
...
def test_cutlass_permutation_dense2():
    b, m, n, k = 2, 128, 128, 128
    assert build(constructPermutationGEMM2(b, m, n, k)), "build failure on CUDA"
    dev = tvm.cuda()
    A = np.random.rand(b, m, k).astype("float16") * 5
    B = np.random.rand(n, k).astype("float16") * 5
    A_tvm = tvm.nd.array(A, dev)
...
# -------------------- debug examples -----------------------
# example of design of R.function() new emit calls

# einsum "ij, ik -> jk"
def constructTransGEMMDebug(m, n, k):
    @tvm.script.ir_module
    class GEMM:
        @T.prim_func
        def gemm(x: T.handle, y: T.handle, z: T.handle) -> None:
        ...
        def main(A: R.Tensor((k, m), A_TYPE), B: R.Tensor((k, n), B_TYPE)):
            with R.dataflow():
                A_trans: R.Tensor((m, k), A_TYPE) = R.permute_dims(A, [1, 0])
                C: R.Tensor((m, n), C_TYPE) = R.call_tir(
                    gemm, (A_trans, B), R.Tensor((m, n), C_TYPE)
                )
                R.output(C)
            return C
...
def test_cutlass_trans_dense_debug():
    ...
    """
    m, n, k = 32, 128, 64
    mod = LegalizeOps()(constructTransGEMMDebug(m, n, k))
...
if __name__ == "__main__":
    ...
    # test_cutlass_multi_batch_dense_bias()
    # test_cutlass_multi_batch_dense2_bias()
    # test_cutlass_trans_dense()
    # test_cutlass_trans_dense2()
    # test_cutlass_trans_dense_bias()
    ...
    test_cutlass_trans_dense_debug()
    print("passed test")
